...
int ILP(Allocation& allo, const int& version){
	...
	for (int i = 0; i < allo.nbItems; i++){
		for (int j = cap - allo.items[i][0]; j >= 0; j--){
			if (isNodeActive[j] == true){		
				arcs.push_back({j, j + allo.items[i][0],i,-1}); 
				isNodeActive[j + allo.items[i][0]] = true;
			}
		}
	}
	...
	for (int i = 0; i < allo.nbBins; i++){
		arcs.push_back({allo.bins[i], cap + 1,-1,i});
		isNodeActive[allo.bins[i]] = true;
	}
		...
		for (int i = 0; i < arcs.size(); i++){
			if (arcs[i][2] >= 0){
				...
				if(arcs[i][3] >= 0){ 
					...
					isArcUsed[i] = model.addVar(0, allo.nbBins, 0, GRB_INTEGER);
				}
			}
		}
