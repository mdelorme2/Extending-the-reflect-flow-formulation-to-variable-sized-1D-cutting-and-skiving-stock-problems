...
int ILP(Allocation& allo, const int& version){
		...
		for (int j = cap - 1; j >= 0; j--){
			if (isNodeActive[j] == true){			
				...
				for(int l = 0; l < allo.nbBins;l++){				
					if (j < allo.bins[l] &&  j + allo.items[i][0] * 2 > allo.bins[l] && j <= allo.bins[l] * 2 - (j + allo.items[i][0] * 2)){
						allo.infos.nbVarS += 1;
	...
	// Print arcs
	cout << arcs.size() << " arcs " << " " << onlyActiveNodesWR.size() << endl;
	...
	try{
		...
		// Symmetry breaking
		/*for (int i = 0; i < allo.nbItems; i++){
			for (int j = i + 1; j < allo.nbItems; j++){
				if (allo.items[j][1] >= allo.items[i][1]) model.addConstr(nbItemUsed[j] >= nbItemUsed[i]); // w1 >= w2 && p1 <= p2 ------> x2 >= x1 
				else if (allo.items[j][0] == allo.items[i][0] && allo.items[j][1] <= allo.items[i][1]) model.addConstr(nbItemUsed[j] <= nbItemUsed[i]); // w1 == w2 && p1 > p2  ------> x2 <= x1
			}
		}*/
