...
int ILP(Allocation& allo, const int& version){
	...
	vector<vector<bool> > isNodeActiveTran (allo.nbBins,vector<bool>(cap + 1,false)); 
	vector<vector<bool> > isNodeActiveDown (allo.nbBins,vector<bool>(cap + 1,false)); 
	...
	for (int i = 0; i < allo.nbItems; i++){
		for (int j = cap - 1; j >= 0; j--){
			if (isNodeActive[j] == true){ 				
				if (j + allo.items[i][0] * 2 <= cap){ // The arc exists as a standard arc, it can be used for a smaller bin with the right transfer arc 
					...
					for(int l = 1; l < allo.nbBins;l++){
						if (2*allo.bins[l] > cap && j < allo.bins[l] &&  j + allo.items[i][0] * 2 > allo.bins[l] && j <= allo.bins[l] * 2 - (j + allo.items[i][0] * 2)){
							isNodeActiveTran[l][j + allo.items[i][0] * 2] = true;
						}
					}
				}
				else{ // only reflected arc for the largest bin
					if (j <= cap * 2 - (j + allo.items[i][0] * 2)){
						allo.infos.nbVarS += 1;
						arcs.push_back({j, cap * 2 - (j + allo.items[i][0] * 2),i,0}); 
						isNodeActiveWR[cap * 2 - (j + allo.items[i][0] * 2)] = true;
					}
					// if the reflected arc would also have been created for a smaller bin
					for(int l = 1; l < allo.nbBins;l++){								
						if (2*allo.bins[l] > cap && j < allo.bins[l] &&  j + allo.items[i][0] * 2 > allo.bins[l] && j <= allo.bins[l] * 2 - (j + allo.items[i][0] * 2)){							
							cout << "arc " << j << " " << allo.bins[l] * 2 - (j + allo.items[i][0] * 2) << " " << i << " " << l << " should have been created " << endl;
							isNodeActiveDown[l][cap * 2 - (j + allo.items[i][0] * 2)] = true;
							isNodeActiveDownT[cap * 2 - (j + allo.items[i][0] * 2)] = true;
						}
					}
				}
			}
	...
	for (int i = 0; i < allo.nbBins; i++){
		if(2*allo.bins[i] > cap){
			isNodeActiveTran[i][allo.bins[i]] = true;
			for(int j = 0; j <= cap; j++){
				if(isNodeActiveTran[i][j]){
					allo.infos.nbVarS += 1;
					cout << "Reflected transfer arc added " << j << " " << 2*allo.bins[i]-j << " -1 " << i << endl;
					arcs.push_back({j,2*allo.bins[i]-j,-1,i});
					isNodeActiveWR[2*allo.bins[i]-j] = true;
				}
				if(isNodeActiveDown[i][j]){
					allo.infos.nbVarS += 1;
					cout << "Downgrade arc added " << j << " " << j-2*(cap-allo.bins[i]) << " -3 " << i << endl;
					arcs.push_back({j,j-2*(cap-allo.bins[i]),-3,i});
					isNodeActiveWR[j-2*(cap-allo.bins[i])] = true;
				}
			}
		}
		else{
	...
	try{
		...
		vector<vector<int > > downgrade(cap+1);
		for (int i = 0; i < arcs.size(); i++){
			if(arcs[i][2] != -3){
			...
			while (arcsUsed[currTail].size() != 0){			
				...
				if(halfBin[1] >= 0){
					...
					if(halfBin[1] == 0 && downgrade[currTail].size() >= 1){
						...
						currTail -= 2*(cap - allo.bins[halfBin[1]]);
					}
					break;
				}
			}
