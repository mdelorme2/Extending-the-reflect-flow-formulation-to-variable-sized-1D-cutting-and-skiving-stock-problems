	...
	for (int i = 0; i < allo.nbItemTypes; i++){
		...
		for (int j = allo.cap - 1; j >= 0; j--){
			if (isNodeActive[j] == true){
				for (int k = 1; k <= allo.items[i][1]; k++){ // put a large number instead of the demand to find back the numbers of John
					if(j + (k-1) * allo.items[i][0] >= allo.cap || done[j + (k-1) * allo.items[i][0]]) break; 			
					arcs.push_back({j + (k-1) * allo.items[i][0], min(allo.cap, j + k * allo.items[i][0]),i}); 
					isNodeActive[min(allo.cap, j + k * allo.items[i][0])] = true;
