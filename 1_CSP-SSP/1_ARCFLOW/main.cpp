...
int ILP(Allocation& allo, const int& version){
	...
	for (int i = 0; i < allo.nbItemTypes; i++){
		...
		for (int j = allo.cap - allo.items[i][0]; j >= 0; j--){
			if (isNodeActive[j] == true){
				for (int k = 1; k <= allo.items[i][1]; k++){ 
					if(j + (k-1) * allo.items[i][0] > allo.cap - allo.items[i][0] || done[j + (k-1) * allo.items[i][0]]) break; 			
	...
	onlyActiveNodes.push_back(allo.cap);
	for (int i = 0; i < onlyActiveNodes.size() - 1; i++){
		arcs.push_back({onlyActiveNodes[i], onlyActiveNodes[i+1],-1});
	}
