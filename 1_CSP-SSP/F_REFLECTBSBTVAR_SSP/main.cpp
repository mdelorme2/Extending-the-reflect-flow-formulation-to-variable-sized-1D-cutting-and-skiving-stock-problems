			...
			if (isNodeActive[j] == true){
				for (int k = 1; k <= allo.items[i][1]; k++){ 
					...
					if(j + k * allo.items[i][0] * 2 <= allo.cap){
						...
						if(j + (k-1) * allo.items[i][0] * 2 <= allo.cap * 2 - (j + k * allo.items[i][0] * 2)){
							...
							if(i == allo.nbItemTypes - 1 || 2*(allo.cap - (j + (k-1) * allo.items[i][0] * 2)) > allo.items[i+1][0] * 2){
						...
						if(!doneR){
							...
							if(org != 0 || i == allo.nbItemTypes - 1 || allo.items[i+1][0] * 2 < allo.cap){
