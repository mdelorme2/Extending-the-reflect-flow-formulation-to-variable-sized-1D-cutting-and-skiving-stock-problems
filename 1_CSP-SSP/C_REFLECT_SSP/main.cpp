...
int ILP(Allocation& allo, const int& version){
	...
	int minVerR = allo.cap; // leftmost node that is the head of a reflected arc
	...
	for (int i = minVerR; i <= allo.cap; i++){
	...
	for (int i = 0; i < onlyActiveNodes.size() - 1; i++){
		arcs.push_back({onlyActiveNodes[i], onlyActiveNodes[i+1],-1,-1});
	}
	...
	try{
		...
		vector<GRBLinExpr> cFLIn(allo.cap + 1,0);
		vector<GRBLinExpr> cFLOut(allo.cap + 1,0);
		...
		isArcUsed.back() = model.addVar(-GRB_INFINITY, GRB_INFINITY, 0, GRB_INTEGER);
		...
		for (int i = 0; i < arcs.size(); i++){		
			...
			if(arcs[i][2] == -1 && arcs[i][3] == -1){ // The arc is a forward loss arc
				cFLIn[arcs[i][1]] += isArcUsed[i];
				cFLOut[arcs[i][0]] += isArcUsed[i];				
			}
			else{ // The arc is not a forward arc
		...
		for (int i = 1; i <= allo.cap; i++){
			if (isNodeActiveWR[i] == true){
				model.addConstr(cOut[i] + cRIn[i] + cFLIn[i] ==  cIn[i] + cFLOut[i]);
				if(i >= minVerR && i < allo.cap) model.addConstr(cFLOut[i] <= cFLIn[i] + cRIn[i]);
			}
		}	
