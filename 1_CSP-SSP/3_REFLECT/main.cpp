...
int ILP(Allocation& allo, const int& version){
	...
	for (int i = 0; i < allo.nbItemTypes; i++){
		vector<bool> done (allo.cap + 1,false);
		for (int j = allo.cap - 1; j >= 0; j--){
			if (isNodeActive[j] == true){
				for (int k = 1; k <= allo.items[i][1]; k++){ 
					if(j + (k-1) * allo.items[i][0] * 2 > allo.cap - 1 || done[j + (k-1) * allo.items[i][0] * 2]) break; 
	...
	for (int i = 0; i < allo.cap; i++){
		if(isNodeActiveWR[i])
			onlyActiveNodesWR.push_back(i);
	}	
	onlyActiveNodesWR.push_back(allo.cap);
