...
int ILP(Allocation& allo, const int& version){
				...
				for (int k = 1; k <= allo.items[i][1]; k++){ 
					...
					for(int l = 0; l < allo.nbBinTypes;l++){				
						if (j + (k-1) * allo.items[i][0] * 2 < allo.bins[l][0] &&  j + k * allo.items[i][0] * 2 > allo.bins[l][0]){
							// If arc (d,e,i,r) such that d <= e 
							if(j + (k-1) * allo.items[i][0] * 2 <= allo.bins[l][0] * 2 - (j + k * allo.items[i][0] * 2)){
								arcs.push_back({j + (k-1) * allo.items[i][0] * 2, allo.bins[l][0] * 2 - (j + k * allo.items[i][0] * 2),i,l}); 
								isNodeActiveWR[allo.bins[l][0] * 2 - (j + k * allo.items[i][0] * 2)] = true;
								minVerR = min(minVerR, allo.bins[l][0] * 2 - (j + k * allo.items[i][0] * 2));
								allo.infos.nbVarS += 1;
							}
							else{ // If arc (d,e,i,r) such that e < d, , add (d,d,i,r) instead, and only add the arc if i is the last item or if item i + 1 would not produce the same arc (d,d,i,r) 
								if(i == allo.nbItemTypes - 1 || 2*(allo.bins[l][0] - (j + (k-1) * allo.items[i][0] * 2)) > allo.items[i+1][0] * 2){
									arcs.push_back({j + (k-1) * allo.items[i][0] * 2, j + (k-1) * allo.items[i][0] * 2,i,l}); 
									isNodeActiveWR[j + (k-1) * allo.items[i][0] * 2] = true;
									minVerR = min(minVerR, j + (k-1) * allo.items[i][0] * 2);	
									allo.infos.nbVarS += 1;					
								}									
							}
							if(l == 0 && allo.items[i][0] < cap && !doneR){
								...
								if(org != 0 || i == allo.nbItemTypes - 1 || allo.items[i+1][0] * 2 < cap){
	...
	// Loss arcs
	for(int l = 0; l < allo.nbBinTypes;l++) isNodeActiveWR[allo.bins[l][0]] = true;	
