...
int ILP(Allocation& allo, const int& version){
	...
	vector<vector<bool> > isNodeActiveDown (allo.nbBinTypes,vector<bool>(cap + 1,false)); 
		...
		for (int j = cap - 1; j >= 0; j--){
			if (isNodeActive[j] == true){
				for (int k = 1; k <= allo.items[i][1]; k++){ 
					...
					if (j + k * allo.items[i][0] * 2 <= cap){ // The arc exists as a standard arc, it can be used for a smaller bin with the right transfer arc 
						...
						for(int l = 1; l < allo.nbBinTypes;l++){
							if (2*allo.bins[l][0] > cap && j + (k-1) * allo.items[i][0] * 2 <= allo.bins[l][0] &&  j + k * allo.items[i][0] * 2 > allo.bins[l][0] && j + (k-1) * allo.items[i][0] * 2 <= allo.bins[l][0] * 2 - (j + k * allo.items[i][0] * 2)){
								isNodeActiveTran[l][j + k * allo.items[i][0] * 2] = true;
							}
						}
					}
						...
						for(int l = 1; l < allo.nbBinTypes;l++){								
							if (2*allo.bins[l][0] > cap && j + (k-1) * allo.items[i][0] * 2 < allo.bins[l][0] &&  j + k * allo.items[i][0] * 2 > allo.bins[l][0] && j + (k-1) * allo.items[i][0] * 2 <= allo.bins[l][0] * 2 - (j + k * allo.items[i][0] * 2)){
								...
								isNodeActiveDown[l][cap * 2 - (j + k * allo.items[i][0] * 2)] = true;
	...
	for (int i = 0; i < allo.nbBinTypes; i++){
		if(2*allo.bins[i][0] > cap){
			isNodeActiveTran[i][allo.bins[i][0]] = true;
			for(int j = 0; j <= cap; j++){
				...
				if(isNodeActiveDown[i][j]){
					cout << "Downgrade arc added " << j << " " << j-2*(cap-allo.bins[i][0]) << " -3 " << i << endl;
					arcs.push_back({j,j-2*(cap-allo.bins[i][0]),-3,i});
					isNodeActiveWR[j-2*(cap-allo.bins[i][0])] = true;
				}
			}
		}
		else{
