...
int ILP(Allocation& allo, const int& version){
		...
		for (int j = cap - 1; j >= 0; j--){
			if (isNodeActive[j] == true){
				for (int k = 1; k <= allo.items[i][1]; k++){ 
					...
					if(j + (k-1) * allo.items[i][0] > cap - allo.items[i][0]){
						isItemTypeShifted[i] = 1;
						if(!done[cap - allo.items[i][0]]){
							arcs.push_back({cap - allo.items[i][0], cap, i,-1});
							done[cap - allo.items[i][0]] = true;
						}
					}
					else{
						arcs.push_back({j + (k-1) * allo.items[i][0], j + k * allo.items[i][0],i,-1}); 
	...
	for (int i = 0; i < allo.nbItemTypes; i++){
		if(isItemTypeShifted[i]){
			lit = min(lit,i);
			isNodeActive[cap - allo.items[i][0]] = 1;
		}
	}
	
	vector<int> onlyActiveNodes;
	for (int i = min(allo.bins.back()[0],cap - allo.items[lit][0]); i <= cap; i++){
		if(isNodeActive[i])
